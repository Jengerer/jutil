...
namespace JUTIL
{
	...
	public:
		...
		static void md5( const String* input, uint8_t* result ) {
			...
			const uint8_t* initial_msg = (const uint8_t*) input->get_string();
			size_t initial_len = input->get_length();
			...
			// cleanup
			BaseAllocator::release( msg );

			// hash is little endian
			uint32_t* dword_result = (uint32_t*) result;
